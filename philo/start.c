...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/10 12:50:17 by fforlini          #+#    #+#             */
/*   Updated: 2022/05/10 13:07:13 by fforlini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_philo	*fill_philo(t_parameters *parameter)
{
	...
	while (++i < parameter->num_of_philos)
	{
		...
		philo[i].n_meals = 0;
		philo[i].l_fork = i;
		philo[i].r_fork = (i + 1) % parameter->num_of_philos;
		pthread_mutex_init(&parameter->forks[i], NULL);
...
int	init_parameters(t_parameters *parameter, char **argv, int ac)
{
	...
	parameter->forks = malloc(sizeof(pthread_mutex_t) * parameter->num_of_philos);
